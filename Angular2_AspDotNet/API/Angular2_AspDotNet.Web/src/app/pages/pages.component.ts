...
import { CustomDynamicMenuService } from '../pages/menu/customdynamicmenu.service';
...
export class PagesComponent implements OnInit {
    ...
    constructor(private _CustomDynamicMenuService: CustomDynamicMenuService, public appSettings: AppSettings, public router: Router) {
        this.settings = this.appSettings.settings;
        if (sessionStorage["skin"]) {
            this.settings.theme.skin = sessionStorage["skin"];
        }
        this._CustomDynamicMenuService.getDefaultUrl(router.url).subscribe((res: string) => {
            if (res != router.url) {

...
import { verticalMenuItems, verticalMenuItemsV1 } from './menu';
...
export class MenuService {
    ...
    public getVerticalMenuItems(): Array<Menu> {
        if (verticalMenuItemsV1.length == 0) {
            this.getMenuByRoleId().subscribe((res: MenuViewModel[]) => {
                for (let dd of res) {
                    verticalMenuItemsV1.push(new Menu(dd.MenuId, dd.Title, dd.RouterLink, dd.Href, dd.Icon, dd.Target, dd.HasSubMenu, dd.ParentMenuId));
                }
                return verticalMenuItemsV1;
            });
        }
        return verticalMenuItemsV1;
    }
    public getMenuByRoleId(): Observable<any> {
        return this.http.get(apiURL + '/role/GetMenuByRoleId').map(response => response.json());
    }

...
var MenuService = /** @class */ (function () {
    function MenuService(location, renderer2, router, http) {
    ...
    MenuService.prototype.getVerticalMenuItems = function () {
        if (menu_1.verticalMenuItemsV1.length == 0) {
            this.getMenuByRoleId().subscribe(function (res) {
                for (var _i = 0, res_1 = res; _i < res_1.length; _i++) {
                    var dd = res_1[_i];
                    menu_1.verticalMenuItemsV1.push(new menu_model_1.Menu(dd.MenuId, dd.Title, dd.RouterLink, dd.Href, dd.Icon, dd.Target, dd.HasSubMenu, dd.ParentMenuId));
                }
                return menu_1.verticalMenuItemsV1;
            });
        }
        return menu_1.verticalMenuItemsV1;
    };
    MenuService.prototype.getMenuByRoleId = function () {
        return this.http.get(apiURL + '/role/GetMenuByRoleId').map(function (response) { return response.json(); });
    };
    ...
    MenuService.prototype.createVerticalMenuItem = function (menu, menuItem) {
        ...
        if (menuItem.hasSubMenu) {
            ...
            this.renderer2.setAttribute(link, 'data-parent', '#menu' + menuItem.parentId);
    ...
    MenuService.prototype.showActiveSubMenu = function (menu) {
        ...
        var routerLink = url; //url.substring(1, url.length);
        if (menu != undefined) {
            var activeMenuItem_1 = menu.filter(function (item) { return item.routerLink === routerLink; });
            if (activeMenuItem_1[0]) {
                var activeLink = document.querySelector("#link" + activeMenuItem_1[0].id);
                ...
                while (this.renderer2.parentNode(parent_1)) {
                    ...
                    if (parent_1.className == 'collapse') {
                        var parentMenu = menu.filter(function (item) { return item.id === activeMenuItem_1[0].parentId; });
